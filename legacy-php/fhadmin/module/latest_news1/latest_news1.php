...
/**************************************************************************
  ...
  * Tabel Latest_news1 dan Latest_news1_kataterkait
...
$tabel_utama = "latest_news1";
$primary_key = "newsid";

$tabel_ukuran = "latest_news1_ukuran";
$primary_key_ukuran = "ukuranid";

$nama_tpl = "latest_news1_";
$judul_halaman = "Latest News";
$path_image = "../../../images/news/";
$path_image = BASE_DIR_UPLOAD_MODULE."news/";
$smarty->assign('path_file_image' , BASE_URL_UPLOAD_MODULE.'news/');
$smarty->assign('path_image' , BASE_URL_UPLOAD_MODULE.'news/');
		...
		if ($msg==""){ 
    		...
    		if (isset($_POST["insert"])){ 
				$SQL1 =  "INSERT INTO 	".$tabel_utama." (metatag, metakeyword, metadescription, newsdate, newstitle, newsshortdesc, newsdescription, newstitle_en, newsshortdesc_en, newsdescription_en, newsstatus)
                               VALUES			('".$db->escape($data_metatag)."','".$db->escape($data_metakeyword)."','".$db->escape($data_metadescription)."','" . $data_newsdate . "', '".$db->escape($data_newstitle)."', '".$db->escape($data_newsshortdesc)."', '".$db->escape($data_newsdescription)."', '".$db->escape($data_newstitle_en)."', '".$db->escape($data_newsshortdesc_en)."', '".$db->escape($data_newsdescription_en)."', '".$db->escape($data_newsstatus)."')";
				$db   -> Execute($SQL1);
				$lastid = $db->Insert_ID();

                for($i=0;$i<count($data_kataterkait);$i++){
                    $SQL = "INSERT INTO `latest_news1_kataterkait`  (".$primary_key.", `kataterkait`) 	
                                VALUES ('".$lastid."', '".$db->escape($data_kataterkait[$i])."')";
                    $db   -> Execute($SQL);
                }

				if ($_FILES["data_mainimage"]["name"] != "") {
					//*** Proses Upload IMAGE - buat instance baru
					$handle_upload = new Upload($_FILES["data_mainimage"]);
					if ($handle_upload->uploaded) {
						$handle_upload->image_resize = true;
						if ($ratioresized == 'Yes') $handle_upload->image_ratio = true;
						if ($widthimage>0) $handle_upload->image_x = $widthimage;
						if ($heightimage>0) $handle_upload->image_y = $heightimage;
						$handle_upload->file_new_name_body = 'img'.$lastid;
						$handle_upload->Process($path_image);
					}
	                
	                if ($handle_upload->processed) {
	                    $nama_gambar = $handle_upload->file_dst_name;
	                    $SQL1 = "UPDATE		 	".$tabel_utama."
	                                  SET				newsmainimage = '".$handle_upload->file_dst_name."'
	                                  WHERE			".$primary_key." = ".$lastid;
                                      
	                    $db -> Execute($SQL1);
	                } else {
	                    // Upload gagal, log error
	                    error_log("Image upload failed for newsid ".$lastid.": ".$handle_upload->error);
	                }
                ...
                //*** Upload image baru
                if ($_FILES["data_mainimage"]["name"] != "") {
                    // Buat instance Upload yang baru untuk proses upload
                    $handle_upload = new Upload($_FILES["data_mainimage"]);
                    if ($handle_upload->uploaded) {
                        $handle_upload->image_resize = true;
                        if ($ratioresized == 'Yes') $handle_upload->image_ratio = true;
                        if ($widthimage>0) $handle_upload->image_x = $widthimage;
                        if ($heightimage>0) $handle_upload->image_y = $heightimage;
                        $handle_upload->file_new_name_body = 'img'.$data_newsid;
                        $handle_upload->Process($path_image);
                    }
                    // var_dump($handle_upload);
                    // die();
                    
                    if ($handle_upload->processed) {
                        //*** hapus file lama jika ada SETELAH upload berhasil
                        if ($RS->fields["newsmainimage"] != "") {
                            @unlink($path_image.$RS->fields["newsmainimage"]);
                            @unlink($path_image.'crop1600-1200_'.$RS->fields["newsmainimage"].'.webp');
		                    @unlink($path_image.'crop800-600_'.$RS->fields["newsmainimage"].'.webp');
		                    @unlink($path_image.'crop600-400_'.$RS->fields["newsmainimage"].'.webp');
		                    @unlink($path_image.'crop525-250_'.$RS->fields["newsmainimage"].'.webp');
		                    
		                    @unlink($path_image.'crop1600-1200_'.$RS->fields["newsmainimage"]);
		                    @unlink($path_image.'crop800-600_'.$RS->fields["newsmainimage"]);
		                    @unlink($path_image.'crop600-400_'.$RS->fields["newsmainimage"]);
		                    @unlink($path_image.'crop525-250_'.$RS->fields["newsmainimage"]);
                        }
                        
                        $tmp_newsmainimage = $handle_upload->file_dst_name;
                        // var_dump($tmp_newsmainimage);
                        // die();
                    } else {
                        // Upload gagal, gunakan image lama
                        error_log("Image upload failed for newsid ".$data_newsid.": ".$handle_upload->error);
                        $tmp_newsmainimage = $RS->fields["newsmainimage"];
                    }
                } 
                ...
                $SQL1 = "UPDATE		 	    ".$tabel_utama."
                              SET			newsdate = '".$data_newsdate."', 
                                            ...
                                            newsstatus = '".$db->escape($data_newsstatus)."'
                              WHERE			".$primary_key." = ".$data_newsid;
                            //   var_dump($SQL1);
                            //   die();
                $db -> Execute($SQL1);
				...
				//*** Kata Terkait
				$SQL1 = "DELETE FROM		latest_news1_kataterkait
                              WHERE			    ".$primary_key." = ".$data_newsid;
                ...
                for($i=0;$i<count($data_kataterkait);$i++){
                    $SQL = "INSERT INTO `latest_news1_kataterkait`  (".$primary_key.", `kataterkait`) 	
                                VALUES ('".$data_newsid."', '".$db->escape($data_kataterkait[$i])."')";
                                // var_dump($SQL);
                                // die();
                    $db -> Execute($SQL);
                }
    ...
    $SQL = "SELECT 	*
	  ...
	  { $total_jumlah_record = $RS->RecordCount();
			...
			$SQL = "SELECT 			P.* ,   GROUP_CONCAT(C.kataterkait SEPARATOR '<br />') as kataterkait_list
                         ...
                         WHERE 	        (P.newstitle like '%".$search."%'
                                                            OR P.newsshortdesc like '%".$search."%'
                                                            OR P.newsdescription like '%".$search."%'
															OR P.newstitle_en like '%".$search."%'
                                                            OR P.newsshortdesc_en like '%".$search."%'
                                                            OR P.newsdescription_en like '%".$search."%')
                                                            ".$tmp_sql."
                         GROUP BY P.newsid ORDER BY		P." . $orderfield . " " . $order . " 	  
