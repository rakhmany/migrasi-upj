				  ...
				  <div class="table-responsive">
					<table class="table table-striped m-b-none">
                      ...
                      <tbody>
						...
						{section name=listing loop=$view.data_bannerid}
						<tr>
                          <td><label class="checkbox m-n i-checks">
                              <input type="checkbox" name="delete[]" value="{$view.data_bannerid[listing]}">
                              <i></i></label></td>
                          <!-- <td><a href="#modal" data-toggle="modal"><i class="fa fa-search-plus text-muted"></i></a></td> -->
                          <td class="text-center">{counter}.</td>
                          <td class="text-center">
                              {$view.data_bannerdate[listing]}
                              {if $view.data_is_expired[listing] eq '1'}<small class="label label-danger">Expired</small>{/if}
                          </td>
                          <td><a href="{$smarty.server.PHP_SELF}?action=detail&data_bannerid={$view.data_bannerid[listing]}">{$view.data_bannertitle[listing]} / <font color="red">{$view.data_bannertitle_en[listing]}</font></a></td>
                          <td class="text-center">{if $view.data_bannerpic[listing] neq ''}<a class="fancybox" target="_blank" href="{$path_file_image}{$view.data_bannerpic[listing]}"><img src="{$path_file_image}{$view.data_bannerpic[listing]}?{$smarty.now}" style="width:120px;" alt="img" /></a>{else}no image{/if}</td>
                          <!-- <td class="text-center">
							{if $smarty.section.listing.total >1}
								{if $smarty.section.listing.last eq false}
									<a href="{php}echo $_SERVER['PHP_SELF'];{/php}?chorder=down&amp;bannerid={$view.data_bannerid[listing]}">Down</a>
								{/if}
								...
								{if $smarty.section.listing.first eq false}
									<a href="{php}echo $_SERVER['PHP_SELF'];{/php}?chorder=up&amp;bannerid={$view.data_bannerid[listing]}">Up</a>
								{/if}
							{/if}
						  </td> -->
                          <td class="text-center">{$view.data_bannertype[listing]}</td>
                          <td class="text-center">{$view.data_bannerstatus[listing]}</td>
                          <td class="text-center"><a href="{$smarty.server.PHP_SELF}?action=detail&data_bannerid={$view.data_bannerid[listing]}" class="btn btn-default btn-xs btn-rounded btn-info"><i class="fa fa-pencil"></i> Edit</a></td>
                        </tr>
                        {/section}
                      </tbody>
                      <tfoot>
