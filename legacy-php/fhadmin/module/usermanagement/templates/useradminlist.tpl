					...
					<form class="form-inline text-right text-center-xs" role="form" method="post" action="{$smarty.server.PHP_SELF}">
						<div class="form-group">
						  ...
						  <select class="input-sm form-control input-s-sm inline v-middle" name="search1">
							...
							{section name=listing loop=$search_fh_usergroupid}
								<option value="{$search_fh_usergroupid[listing]}"  {if $search1 eq $search_fh_usergroupid[listing]} selected {/if} >{$search_fh_usergroupname[listing]}</option>
							{/section}
						  </select>
						</div>
					...
					<table class="table table-striped m-b-none">
                      <thead>
                        ...
                        <tr>
                          ...
                          <th width="15%" class="th-sortable"><a href="{$smarty.server.PHP_SELF}?action={$view.action}&page={$view.page}&order={if $view.order eq 'asc'}desc{else}asc{/if}&orderfield=fh_username&search={$view.search}&search1={$view.search1}&search2={$view.search2}">Username<span class="th-sort"> <i class="fa fa-sort-down text{if $view.order eq 'desc'}-active{/if}"></i> <i class="fa fa-sort-up text{if $view.order eq 'asc'}-active{/if}"></i> </span></a> </th>
                          <th class="th-sortable"><a href="{$smarty.server.PHP_SELF}?action={$view.action}&page={$view.page}&order={if $view.order eq 'asc'}desc{else}asc{/if}&orderfield=fh_name&search={$view.search}&search1={$view.search1}&search2={$view.search2}">Name<span class="th-sort"> <i class="fa fa-sort-down text{if $view.order eq 'desc'}-active{/if}"></i> <i class="fa fa-sort-up text{if $view.order eq 'asc'}-active{/if}"></i> </span></a> </th>
                          <th width="15%" class="th-sortable"><a href="{$smarty.server.PHP_SELF}?action={$view.action}&page={$view.page}&order={if $view.order eq 'asc'}desc{else}asc{/if}&orderfield=fh_usergroupid&search={$view.search}&search1={$view.search1}&search2={$view.search2}">User Group<span class="th-sort"> <i class="fa fa-sort-down text{if $view.order eq 'desc'}-active{/if}"></i> <i class="fa fa-sort-up text{if $view.order eq 'asc'}-active{/if}"></i> </span></a> </th>
                          <th width="10%" class="th-sortable"><a href="{$smarty.server.PHP_SELF}?action={$view.action}&page={$view.page}&order={if $view.order eq 'asc'}desc{else}asc{/if}&orderfield=fh_logindate&search={$view.search}&search1={$view.search1}&search2={$view.search2}">Login Date<span class="th-sort"> <i class="fa fa-sort-down text{if $view.order eq 'desc'}-active{/if}"></i> <i class="fa fa-sort-up text{if $view.order eq 'asc'}-active{/if}"></i> </span></a> </th>
                          <th width="10%" class="text-center">Actions</th>
                        </tr>
                      </thead>
                      <tbody>
						{math equation='(cc*y)-cc' cc=$conf.page y=$view.page assign='ctr'}{counter start=$ctr print=false}
						{section name=listing loop=$view.fh_userid_temp}
						<tr>
                          <td><label class="checkbox m-n i-checks">
                              <input type="checkbox" name="delete[]" value="{$view.fh_userid_temp[listing]}">
                              <i></i></label></td>
                          <td class="text-center">{counter}.</td>
						  <td>{$view.fh_username_temp[listing]}</td>
                          <td><a href="{$smarty.server.PHP_SELF}?action=detail&data_fh_userid={$view.fh_userid_temp[listing]}">{$view.fh_name_temp[listing]}</a></td>
                          <td>{$view.fh_usergroupname_temp[listing]}</td>
                          <td>{$view.fh_logindate_temp[listing]}</td>
                          <td class="text-center"><a href="{$smarty.server.PHP_SELF}?action=detail&data_fh_userid={$view.fh_userid_temp[listing]}" class="btn btn-default btn-xs btn-rounded btn-info"><i class="fa fa-pencil"></i> Edit</a></td>
                        </tr>
                        {/section}
                      </tbody>
                      <tfoot>
