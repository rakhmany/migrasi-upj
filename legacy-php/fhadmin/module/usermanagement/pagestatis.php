...
$judul_halaman = "Static Page";
//print_r($width_imagelist);

$smarty->assign('width', $width_imagelist);
$smarty->assign('height', $height_imagelist);
if ($fh_userid && $akses_level_page)
{	$fh_cpagestatis = new fh_pagestatis();

    $optarray_menucat = enum($tabel_utama.".fh_menu_cat");
    $smarty->assign('optarray_menucat' , $optarray_menucat);
    
    
    $optarray_kolomtype = array('1 kolom', '2 kolom', '3 kolom');
    $smarty->assign('optarray_kolomtype' , $optarray_kolomtype);
    ...
    // $smarty->display('pagestatisheader.tpl');

    //*** Add New Menu
    if (isset($_POST["insertmenu"])) {
        if (!$access_level_type->add) akses_level_die();
        $tmp_data = $fh_cpagestatis->insert_menu($_POST, $_FILES, $path_image, $width_imagelist, $height_imagelist);
    ...
    if (isset($_POST["editmenu"])) {
        ...
        $tmp_data = $fh_cpagestatis->edit_menu($_POST, $_FILES, $path_image, $width_imagelist, $height_imagelist);
        if ($tmp_data["msg2"] == "") {
    ...
    switch ($action){
        case "view"   :
            $tmp_menu = $fh_cpagestatis->select_deep(0,"----");

            $smarty->assign('msg' , $tmp_data["msg"]);
            $smarty->assign('strukturmenu' , $tmp_menu);
            $smarty->display('pagestatislist.tpl');
            break;


        case "insert" :
            $tmp_menu = $fh_cpagestatis->select_deep_core(0,"----", $tmp_data["data_coremenu"]);
            ...
            $smarty->assign('fh_menu_pageheader_en' , $tmp_data["data_fh_menu_pageheader_en"]);
            ...
            $smarty->assign('fh_content_title_en' , $tmp_data["data_fh_content_title_en"]);
            $smarty->assign('fh_content_description_en' , $tmp_data["data_fh_content_description_en"]);
            $smarty->assign('fh_modulefilename' , $tmp_data["data_fh_modulefilename"]);
            $smarty->assign('fh_menu_cat' , $tmp_data["data_fh_menu_cat"]);
            $smarty->assign('fh_coloumn_count' , $tmp_data["data_fh_coloumn_count"]);
            $smarty->assign('fh_content_description2' , $tmp_data["data_fh_content_description2"]);
            $smarty->assign('fh_content_description2_en' , $tmp_data["data_fh_content_description2_en"]);
            $smarty->assign('fh_content_description3' , $tmp_data["data_fh_content_description3"]);
            $smarty->assign('fh_content_description3_en' , $tmp_data["data_fh_content_description3_en"]);
             
            $smarty->assign('fh_menu_catselected' , explode(':::', $tmp_data["data_fh_menu_catselected"]));
            $smarty->assign('strukturmenu' , $tmp_menu);
            ...
            $smarty->display('pagestatisinsert.tpl');
            break;
        ...
        case "detail" :
            if ((isset($_GET["coremenu"])) and (ctype_digit($_GET["coremenu"])) and ($_GET["coremenu"] > 0))  {
                ...
                $tmp_menu = $fh_cpagestatis->select_deep_core(0,"----", $RS->fields["fh_strukturparent"]);
                $optarray_strukturparenttipe = enum($tabel_utama.".fh_strukturparenttipe");
                $optarray_strukturcontentstatus = enum($tabel_utama.".fh_strukturstatus");
                $optarray_strukturcontenttipe = enum($tabel_utama.".fh_strukturtipe");
                ...
                $smarty->assign('fh_menu_pageheader_en' , $RS->fields["fh_menu_pageheader_en"]);
                ...
                $smarty->assign('fh_content_title_en' , $RS->fields["fh_content_titlename_en"]);
                $smarty->assign('fh_content_description_en' , $RS->fields["fh_content_description_en"]);
                $smarty->assign('fh_modulefilename' , $RS->fields["fh_modulefilename"]);
                $smarty->assign('fh_menu_cat' , $RS->fields["fh_menu_cat"]);
                $smarty->assign('fh_coloumn_count' , $RS->fields["fh_coloumn_count"]);
	            $smarty->assign('fh_content_description2' , $RS->fields["fh_content_description2"]);
	            $smarty->assign('fh_content_description2_en' , $RS->fields["fh_content_description2_en"]);
	            $smarty->assign('fh_content_description3' , $RS->fields["fh_content_description3"]);
	            $smarty->assign('fh_content_description3_en' , $RS->fields["fh_content_description3_en"]);
            	
	            
	            $SQL = "SELECT      * FROM         fh_pagestatis_kataterkait
                             WHERE      fh_strukturid = '".$_GET["coremenu"]."' ";
                $RS2 = $db->Execute($SQL);
                $i = 0;
                while (!$RS2->EOF) {
                ...
                $smarty->assign('fh_menu_catselected' , explode(':::', $RS->fields["fh_menu_catselected"]));
                $smarty->assign('strukturmenu' , $tmp_menu);
                $smarty->assign('width_imagelist' , $width_imagelist);
                $smarty->assign('height_imagelist' , $height_imagelist);
                $smarty->assign('path_image' , $path_image_url);
