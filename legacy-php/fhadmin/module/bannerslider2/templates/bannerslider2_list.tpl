                      ...
                      <tbody>
						...
						<tr>
                          <td><label class="checkbox m-n i-checks">
                              <input type="checkbox" name="delete[]" value="{$view.data_newsid[listing]}">
                              <i></i></label></td>
                          <!-- <td><a href="#modal" data-toggle="modal"><i class="fa fa-search-plus text-muted"></i></a></td> -->
                          <td class="text-center">{counter}.</td>
                          <td class="text-center">{$view.data_newsdate[listing]}</td>
                          <td><a href="{$smarty.server.PHP_SELF}?action=detail&data_newsid={$view.data_newsid[listing]}">{$view.data_newstitle[listing]}</a></td>
                          <td class="text-center">{if $view.data_newsbg[listing] neq ''}<a class="fancybox" target="_blank" href="{$path_file_image}{$view.data_newsbg[listing]}"><img src="{$path_file_image}{$view.data_newsbg[listing]}?{$smarty.now}" style="width:120px;" alt="img" /></a>{else}no image{/if}</td>
                          <td class="text-center">
							{if $smarty.section.listing.total >1}
								{if $smarty.section.listing.last eq false}
									<a href="{php}echo $_SERVER['PHP_SELF'];{/php}?chorder=down&amp;newsid={$view.data_newsid[listing]}">Down</a>
								{/if}
								...
								{if $smarty.section.listing.first eq false}
									<a href="{php}echo $_SERVER['PHP_SELF'];{/php}?chorder=up&amp;newsid={$view.data_newsid[listing]}">Up</a>
								{/if}
							{/if}
						  </td>
                          <td class="text-center"><span class="label bg-{if $view.data_newsstatus[listing] eq 'Hidden'}danger{else}success{/if}">{$view.data_newsstatus[listing]}</span></td>
                          <td class="text-center"><a href="{$smarty.server.PHP_SELF}?action=detail&data_newsid={$view.data_newsid[listing]}" class="btn btn-default btn-xs btn-rounded btn-info"><i class="fa fa-pencil"></i> Edit</a></td>
                        </tr>
                        {/section}
                      </tbody>
                      <tfoot>
              ...
              <footer class="footer bg-white b-t">
                <div class="row text-center-xs">
                  ...
                  <div class="col-md-3 hidden-sm">
                    <p class="text-muted m-t">&nbsp;Total: {$view.total_jumlah_record}</p>
                  </div>
