        ...
        <section class="hbox stretch">
          ...
          <aside>
            <section class="vbox">
              ...
              <p class="h4 font-thin pull-left m-r m-b-sm">{$title}</p>
				  ...
				  <div class="table-responsive">
					<table class="table table-striped m-b-none">
                      ...
                      <tbody>
						...
						{section name=listing loop=$view.data_logoid}
						<tr>
                          <td><label class="checkbox m-n i-checks">
                              <input type="checkbox" name="delete[]" value="{$view.data_logoid[listing]}">
                              <i></i></label></td>
                          <!-- <td><a href="#modal" data-toggle="modal"><i class="fa fa-search-plus text-muted"></i></a></td> -->
                          <td class="text-center">{counter}.</td>
                          <td><a href="{$smarty.server.PHP_SELF}?action=detail&data_logoid={$view.data_logoid[listing]}">{$view.data_logotitle[listing]}</a></td>
                          <td class="text-center">{if $view.data_logofile[listing] neq ''}<a class="fancybox" href="{$path_file_image}{$view.data_logofile[listing]}"><img src="{$path_file_image}{$view.data_logofile[listing]}" style="width:120px;" alt="img" /></a>{else}no image{/if}</td>
                          <td class="text-center"><span><a class="label bg-{if $view.data_logostatus[listing] eq 'Hidden'}danger{else}success{/if}" href="{php}echo $_SERVER['PHP_SELF']; {/php}?action=view&status=show&data_logoid={$view.data_logoid[listing]}" title="Hide / Show"><i class="fa fa-eye"></i> {$view.data_logostatus[listing]}</a></span></td>
                          <td class="text-center"><a href="{$smarty.server.PHP_SELF}?action=detail&data_logoid={$view.data_logoid[listing]}" class="btn btn-default btn-xs btn-rounded btn-info"><i class="fa fa-pencil"></i> Edit</a></td>
                        </tr>
                        {/section}
                      </tbody>
                      <tfoot>
