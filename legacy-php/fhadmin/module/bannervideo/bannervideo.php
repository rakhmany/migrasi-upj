...
{	
	...
	{ 
	  ...
	  if( isset($data_mainpdf["name"]) && $data_mainpdf['name'] !='')
	  {
		   	$handle                     = new Upload($data_mainpdf);
	        $handle->allowed            = array('video/mp4'); 
	        if ($handle->uploaded) { 
	            $handle->Process("../../../upload/");
	            if ($handle->processed) {
		   ...
		   if (isset($_POST["insert"])) 
	  	   {
			   ...
			   WHERE ".$primarykey." = ".$data_primarykey;
				...
				if (  isset($mainpdfname) && $mainpdfname != '') {
		  ...
		  if($allowmainimage === true)
		  {
	  	  	  ...
	  	  	  if(isset($RS->fields["mainpdfname"]) && $RS->fields["mainpdfname"] != '' && file_exists("../../../upload/".$RS->fields["mainpdfname"]) )
			  {
				 $oldpdf = '<div class="row"> 
	                        ...
	                        <br />
	                        <a href="pdf.php?remove&i='.$RS->fields[$primarykey].'&p='.$primarykey.'&t='.$maintable.'&f='.basename(__FILE__).'" style="color: blue;"><span class="label bg-danger">Remove Video</span></a>
	                        </div> 
	                        </div></div>'; 
			  } 
