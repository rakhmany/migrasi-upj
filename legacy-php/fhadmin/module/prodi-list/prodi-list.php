...
{	
	...
	$option_level_val = array();
	$option_level_name = array();
	$option_level_arr = array();
	...
	if ($RS->fields['content_id'] != "")
	{
		...
		while (!$RS->EOF)  
		{ 
			$option_level_val[$i] = $RS->fields['content_id'];
			$option_level_name[$i] = $RS->fields['content_name_id'];
			$option_level_arr[$RS->fields['content_id']] = $RS->fields['content_name_id'];
			$i++;
			$RS->MoveNext();  
		} 
	}
	$smarty->assign('option_level_val' ,  $option_level_val);
	$smarty->assign('option_level_name' ,  $option_level_name);
	
	$option_interest_val = array();
	$option_interest_name = array();
	$option_interest_arr = array();
	...
	{		
		...
		while (!$RS->EOF)  
		{ 
			$option_interest_val[$i] = $RS->fields['content_id'];
			$option_interest_name[$i] = $RS->fields['content_name_id'];
			$option_interest_arr[$RS->fields['content_id']] = $RS->fields['content_name_id'];
			$i++;
			$RS->MoveNext();  
		} 
	}
	$smarty->assign('option_interest_val' ,  $option_interest_val);
	$smarty->assign('option_interest_name' ,  $option_interest_name);
	...
	{ 
	  ...
	  $data_content_testi_id 	= trim($_POST['data_content_testi_id']);
	  $data_content_testi_en  	= trim($_POST['data_content_testi_en']);
	  $data_content_url  	= trim($_POST['data_content_url']);
	 ...
	 $data_d_newsmain_date_sql =   date('Y-m-d ', strtotime($data_d_newsmain_date)).date('H:i:s');
	  ...
	  if ($msg=="")
	  {
		  ...
		  $data_content_permalink = createPermaKey($maintable, $data_content_title_en);
		   ...
		   if (isset($_POST["insert"])) 
	  	   {
			   ...
			   content_testi_id='".mysql_real_escape_string($data_content_testi_id)."',  
			   content_testi_en='".mysql_real_escape_string($data_content_testi_en)."' ,  
			   content_url='".mysql_real_escape_string($data_content_url)."' ,  
			   list_priority='".mysql_real_escape_string($max['value'])."'  ";
			   $db   -> Execute($SQL1); 
			   $data_primarykey = $db->Insert_Id();
			   
			   $logword = 'Add';
			   
			    if(is_array($data_level) && sizeof($data_level) > 0 )
			    {
				    foreach($data_level as $key => $val)
					{
						$SQL1 = 'INSERT INTO prodi_filter SET filter_type = 1  , prodi_id = '.$data_primarykey.' , prodi_filter='.$val.' ';
		               	$db -> Execute($SQL1); 
					}
				}
				if(is_array($data_interest) && sizeof($data_interest) > 0 )
			    {
				    foreach($data_interest as $key => $val)
					{
						$SQL1 = 'INSERT INTO prodi_filter SET filter_type = 2  , prodi_id = '.$data_primarykey.' , prodi_filter='.$val.' ';
		               	$db -> Execute($SQL1); 
					}
				} 
			   ...
			   WHERE ".$primarykey." = ".$data_primarykey;
				...
				$SQL1 = 'DELETE FROM prodi_filter WHERE filter_type = 1  AND  prodi_id = '.$data_primarykey.'   ';
		        $db -> Execute($SQL1);  
				if(is_array($data_level) && sizeof($data_level) > 0 )
			    {
				    foreach($data_level as $key => $val)
				...
				$SQL1 = 'DELETE FROM prodi_filter WHERE filter_type = 2  AND  prodi_id = '.$data_primarykey.'   ';
		        $db -> Execute($SQL1); 
				if(is_array($data_interest) && sizeof($data_interest) > 0 )
			    {
				    foreach($data_interest as $key => $val)
	...
	$order = " asc ";
	  $sql_cond = '';
	  if ($search!="") {
		  $sql_cond = " WHERE 	".$maintable.".content_title_id like '%".$search."%' OR ".$maintable.".content_title_en  like '%".$search."%'   ";
	  }
	  
	  $SQL = "SELECT 	*
	  	   	  FROM		".$maintable." 
			   ".$sql_cond." ORDER BY	".$orderfield."  ".$order ."";
		...
		if ($page != 0 ) {
	      ...
	      while (!$RS->EOF) {
		      ...
		      $link_p = isset($_GET['page']) ? 'page='.$_GET['page'] : '';
	  	      ...
	  	      $timestamp_temp[$i] = date("d-m-Y" , strtotime($RS->fields['content_time']) ) ; 
	  	      $mainimagename_temp[$i] = file_exists("../../../upload/".$RS->fields['mainimagename']) ? '<img src="../../../upload/thumb_'.$RS->fields['mainimagename'].'" style="width: 100px;" /> ' : 'N/A' ; 
	  ...
	  $smarty->assign('view', array(
	  			...
	  			'mainimagename_temp' 			=> $mainimagename_temp,
	  ...
	  if(is_array($RS->fields) && sizeof($RS->fields)>0)
	  {
		  ...
		  if($allowmainimage === true)
		  {
			  ...
			  $smarty->assign('oldimage' 	, $oldimage);
	  	  	...
	  	  	$sql = ' SELECT * FROM `prodi_filter` WHERE prodi_id=\''.$data_primarykey.'\'   ORDER BY  `filter_type` ASC     ';
			$RS = $db->Execute($sql); 
			if ($RS->fields['filter_id'] != "")
			{		 
				while (!$RS->EOF)  
				{
					if($RS->fields['filter_type'] == '1')
					{
						$data_level[] = $RS->fields['prodi_filter'];
					}
					if($RS->fields['filter_type'] == '2')
					{
						$data_interest[] = $RS->fields['prodi_filter'];
					} 
					$RS->MoveNext();  
				} 
			}
