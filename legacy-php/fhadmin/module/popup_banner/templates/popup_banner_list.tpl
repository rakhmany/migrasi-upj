				  ...
				  <div class="table-responsive">
					<table class="table table-striped m-b-none">
                      ...
                      <tbody>
						...
						{section name=listing loop=$view.data_pbanner_id}
						<tr>
                          <td><label class="checkbox m-n i-checks">
                              <input type="checkbox" name="delete[]" value="{$view.data_pbanner_id[listing]}">
                              <i></i></label></td>
                          <!-- <td><a href="#modal" data-toggle="modal"><i class="fa fa-search-plus text-muted"></i></a></td> -->
                          <td class="text-center">{counter}.</td>
                          <td class="text-center">{$view.data_pbanner_date[listing]}</td>
                          <td class="text-center">{$view.data_pbanner_date_end[listing]}</td>
                          <td><a href="{$smarty.server.PHP_SELF}?action=detail&data_pbanner_id={$view.data_pbanner_id[listing]}">{$view.data_pbanner_title[listing]}</a></td>
                          <td class="text-center">{if $view.data_pbanner_pic[listing] neq ''}<a class="fancybox" href="{$path_file_image}{$view.data_pbanner_pic[listing]}"><img src="{$path_file_image}{$view.data_pbanner_pic[listing]}?{$smarty.now}" style="width:120px;" alt="img" /></a>{else}no image{/if}</td>
                          <td class="text-center">
							{if $smarty.section.listing.total >1}
								{if $smarty.section.listing.last eq false}
									<a href="{php}echo $_SERVER['PHP_SELF'];{/php}?chorder=down&amp;pbanner_id={$view.data_pbanner_id[listing]}">Down</a>
								{/if}
								...
								{if $smarty.section.listing.first eq false}
									<a href="{php}echo $_SERVER['PHP_SELF'];{/php}?chorder=up&amp;pbanner_id={$view.data_pbanner_id[listing]}">Up</a>
								{/if}
							{/if}
						  </td>
                          <td class="text-center"><span class="label bg-{if $view.data_pbanner_status[listing] eq 'Hidden'}danger{else}success{/if}">{$view.data_pbanner_status[listing]}</span></td>
                          <td class="text-center"><a href="{$smarty.server.PHP_SELF}?action=detail&data_pbanner_id={$view.data_pbanner_id[listing]}" class="btn btn-default btn-xs btn-rounded btn-info"><i class="fa fa-pencil"></i> Edit</a></td>
                        </tr>
                        {/section}
                      </tbody>
                      <tfoot>
