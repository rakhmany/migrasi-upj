...
$maintable = 'd_footercertificate';
$primarykey = 'd_slideshow_id';

$mainfieldname = 'd_slideshow_name_id';
$mainfieldnamed = 'ISO Name';
$systemname = 'Footer Certificate';
...
{	
	...
	case "search" :
	  if ($search!="") {
	    $SQL = "SELECT 	*
	  	   	    FROM	".$maintable."
	  	   	    WHERE 	".$mainfieldname." like '%".$search."%'  
			    ORDER BY list_priority ASC ";
	    $RS =$db->Execute($SQL);
	    if ($RS->fields[$primarykey] != "")
	    { $total_jumlah_record = $RS->RecordCount();
	  ...
	  } else $msg = "Please input keyword for searching<br>";
