...
$imagethumbsize_x = 500;
$imagethumbsize_y = 850;
...
{	
	...
	$option_cat_arr[0] = '';
	$i++;
	$sqlc = 'SELECT * FROM  d_about_team_category  ORDER BY `priority` ASC;'; 
	$RS =$db->Execute($sqlc);
	...
	{ 
	  ...
	  $data_d_content_icon 			= trim($_POST["data_d_content_icon"]); 
		   ...
		   if (isset($_POST["insert"])) 
	  	   {
			   ...
			   d_content_icon='".mysql_real_escape_string($data_d_content_icon)."',   
	...
	if($orderfield == 'priority')
	{
	  ...
	  $sql_cond = '';
	  if ($search!="") {
		  $sql_cond = " WHERE 	".$maintable.".".$mainfieldname." like '%".$search."%'  OR 	".$maintable.".d_content_year like '%".$search."%'   ";
	  }
