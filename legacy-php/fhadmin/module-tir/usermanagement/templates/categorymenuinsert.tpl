      ...
      <section id="content">
        <section class="vbox">
          <header class="header bg-info b-b clearfix">
			<div class="row m-t-sm">
			  <div class="col-sm-5 m-b-xs">
				...
				<span class="h5">{if $data_fh_kategorimenuid neq ''}Edit{else}Insert{/if} {$title}</span>
			  </div>
          ...
          <section class="scrollable wrapper w-f">
            ...
            <div class="row">
				<div class="col-lg-12">
				  ...
				  <ul class="breadcrumb">
					...
					<li class="active"><i class="fa fa-pencil"></i> {if $data_fh_kategorimenuid neq ''}Edit{else}Insert{/if}</li>
				  </ul>
				  <!-- / .breadcrumb -->
				</div>
            </div>
			...
			<form class="form-horizontal" data-validate="parsley" name="form" method="post" enctype="multipart/form-data" action="{$smarty.server.PHP_SELF}{if $data_fh_kategorimenuid neq ''}?data_fh_kategorimenuid={$data_fh_kategorimenuid}{/if}">
            <div class="row">
              <div class="col-sm-12">
                  <section class="panel panel-default">
                    <header class="panel-heading"> <strong>Basic Information</strong> </header>
                    <div class="panel-body">
                      {if $data_fh_kategorimenuid neq ''}
					  <div class="form-group">
                        <label class="col-sm-2 control-label">ID</label>
                        <div class="col-sm-6">
                          <input type="hidden" name="data_fh_kategorimenuid" value="{$data_fh_kategorimenuid}">{$data_fh_kategorimenuid}
                        </div>
                      </div>
