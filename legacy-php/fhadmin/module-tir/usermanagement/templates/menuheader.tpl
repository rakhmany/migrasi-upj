<html>
<head>
	  <title>FaberCMS</title>
	  <meta name="author" content="FaberHost Indonesia - Seto Andry Wibowo">
	  <meta name="copyright" content="FaberHost Indonesia">
	  <meta name="creator" content="Webmaster">
</head>
<link rel="stylesheet" href="../../css.css" type="text/css">
</head>

<body>

<div id=worksheet>
  <div id=header>
  <h1><span><img src=../../images/icon1.gif> Seting Menu CMS</span></h1>
  <hr class=black>

  <form method=post action={php} echo $_SERVER['PHP_SELF']; {/php}>
    <span>Search Menu : <input type=text name=search size=20> 

    <select name=search1>
    <option value=''>== ALL ==</option>
    {section name=listing loop=$search_fh_kategorimenuid}
    <option value='{$search_fh_kategorimenuid[listing]}'>{$search_fh_name[listing]}</option>
    {/section}    
    </select>

    <input type=submit value=' search '> &nbsp;{$insert} &nbsp;{$view}
    </span>
  </form>
  <hr class=black>
  </div>
</div>