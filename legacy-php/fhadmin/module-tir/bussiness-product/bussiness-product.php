...
$imagethumbsize_x = 970;
$imagethumbsize_y = 500;
$best_image_view = '<br />Best view: '.($imagethumbsize_x ).'px - '.($imagethumbsize_y ).'px';
$smarty->assign('best_image_view' ,  $best_image_view);

$imagethumbsize_x3 = 476;
$imagethumbsize_y3 = 400;
$best_image_view = '<br />Best view: '.($imagethumbsize_x3 ).'px - '.($imagethumbsize_y3 ).'px';
...
{	
	...
	$option_cat_arr[0] = '';
	$i++;
	$sqlc = 'SELECT * FROM  d_subsidiaries WHERE d_content_ischild=0  ORDER BY `priority` ASC;'; 
	$RS =$db->Execute($sqlc);
	if ($RS->fields['d_content_id'] != "")
	{   
		while (!$RS->EOF) {
		    $option_cat_val[$i] = $RS->fields['d_content_id'];
			$option_cat_name[$i] = $RS->fields['d_content_title_id'];
			$option_cat_arr[$RS->fields['d_content_id']] = $RS->fields['d_content_title_id'];
			$i++;
		    $RS->MoveNext();
	  	  }
	} 
	...
	$search_category = isset($_POST['search_category']) ? $_POST['search_category']: (isset($_GET['search_category']) ? $_GET['search_category']: false) ;
	...
	case "view" :
	case "search" :
	if(!empty($search)   ||   $search_category > 0 )
	{
	...
	if($orderfield == 'priority')
	{
	  ...
	  $sql_cond = ' WHERE 1=1';
	  if ($search!="") {
		  $sql_cond .= " AND 	".$maintable.".".$mainfieldname." like '%".$search."%'     ";
	  }
	  if ($search_category > 0 ) {
		  $sql_cond .= " AND 	".$maintable.".d_content_parentcomp like '".$search_category."'     ";
	  }
	  
	  $SQL = "SELECT 	".$maintable.".* 
	  	   	  FROM		".$maintable." 
	  	   	  LEFT JOIN d_subsidiaries ON d_subsidiaries.d_content_id=".$maintable.".d_content_parentcomp
	  	   	  ".$sql_cond."
	      ...
	      while (!$RS->EOF) {
			...
			$this_priority = $link_down.$link_up == '' ? '--' : $link_down.' '.$link_up;
		      ...
		      $primarykey_temp[$i] = $RS->fields[$primarykey];
	  	      ...
	  	      $gennews_statname_temp[$i] = $option_cat_arr[$RS->fields['d_content_parentcomp']]; 
		  ...
		  else
		  {
			  if(is_array($RS->fields) && sizeof($RS->fields)>0)
			  {
				  ...
				  {   $data_gallery_image = $_FILES['data_gallery_image'];
					  ...
					  { $data_gallery_name = 'Image Gallery'; }
					  ...
					  if($errmsg == '')
					  {  $sql = 'INSERT INTO  `d_companyvalues_gallery` SET `'.$primarykey.'`=\''.$data_primarykey.'\', `gallery_image`=\''.$mainimagename.'\',  `gallery_name`=\''.$data_gallery_name.'\' ;';
				  ...
				  if(isset($_GET['delete_img']))
			  	  {  $old_mainimage = $_GET['old_mainimage'];
				  	  ...
				  	  $sql = 'DELETE FROM `d_companyvalues_gallery`  WHERE gallery_id=\''.intval($_GET['delete_img']).'\' ;';
			  	  ...
			  	  {
				  	  $sql = 'UPDATE `d_companyvalues_gallery`  SET gallery_ismain=0 WHERE `'.$primarykey.'` =\''.$data_primarykey.'\'   ;';
					  if(@mysql_query($sql)) { 
						  	$sql = 'UPDATE `d_companyvalues_gallery`  SET gallery_ismain=1 WHERE `'.$primarykey.'` =\''.$data_primarykey.'\'  and gallery_id=\''.intval($_GET['setmain_img']).'\' ;';
			  	  ...
			  	  //*** View Gallery
			  	  $sql = 'SELECT * FROM `d_companyvalues_gallery` WHERE `'.$primarykey.'` =\''.$data_primarykey.'\'   ORDER BY gallery_id ASC ';
			  	  $res = mysql_query($sql);
			  	  $num = @mysql_num_rows($res);
			  	  if($num)
			  	  {  $i=0;
	...
	case "subcontent" :
	 ...
	 $imagethumbsize_x = 370;
	$imagethumbsize_y = 250;
	$best_image_view = '<br />Best view: '.($imagethumbsize_x ).'px - '.($imagethumbsize_y ).'px';
	$smarty->assign('best_image_view' ,  $best_image_view);
		  ...
		  else
		  {
			  if(is_array($RS->fields) && sizeof($RS->fields)>0)
			  {
				  ...
				  { 
					  ...
					  $data_gallery_description_id = $_POST['data_gallery_description_id'];
					  $data_gallery_description_en = $_POST['data_gallery_description_en'];
					  ...
					  if($data_gallery_description_id == '' || $data_gallery_description_en == '' )
					  { $errmsg.="Please input description<br>"; }
					  ...
					  if($errmsg == '')
					  {
						  if(isset($_POST['submitedit']))
						  {
							  $sql = 'UPDATE  `d_companyvalues_subcontent` SET  `gallery_image`=\''.$mainimagename.'\',  `gallery_name_id`=\''.$data_gallery_name_id.'\',  `gallery_description_id`=\''.$data_gallery_description_id.'\',  `gallery_name_en`=\''.$data_gallery_name_en.'\',  `gallery_description_en`=\''.$data_gallery_description_en.'\' 
							  			WHERE gallery_id=\''.$edit_id.'\' 
							  			;';
							  if(@mysql_query($sql)) {  $errmsg .= 'Services  has been updated'; }
							  else {  $errmsg .= 'Services cant been updated.'; }
						  }
						  else
						  {
							  $sql = 'INSERT INTO  `d_companyvalues_subcontent` SET `'.$primarykey.'`=\''.$data_primarykey.'\', `gallery_image`=\''.$mainimagename.'\',  `gallery_name_id`=\''.$data_gallery_name_id.'\',  `gallery_description_id`=\''.$data_gallery_description_id.'\',  `gallery_name_en`=\''.$data_gallery_name_en.'\',  `gallery_description_en`=\''.$data_gallery_description_en.'\' ;';
							  if(@mysql_query($sql)) {  $errmsg .= 'New services has been inserted'; }
							  else {  $errmsg .= 'New services cant been inserted.'; }
						  }
				  ...
				  if(isset($_GET['delete_img']))
			  	  {   
				  	  $sql = 'DELETE FROM `d_companyvalues_subcontent`  WHERE gallery_id=\''.intval($_GET['delete_img']).'\' ;';
			  	  ...
			  	  {   
				  	  $sql = 'SELECT * FROM `d_companyvalues_subcontent`  WHERE gallery_id=\''.intval($_GET['edit_img']).'\' ;';
					  $res = mysql_query($sql);
				  	  $num = @mysql_num_rows($res);
				  	  if($num)
				  	  {
						  ...
						  $data_gallery_image = $row['gallery_image'];
						  $data_gallery_description_id = $row['gallery_description_id'];
						  $data_gallery_description_en = $row['gallery_description_en'];
						  $data_gallery_name_id = $row['gallery_name_id'];
						  $data_gallery_name_en = $row['gallery_name_en'];
						  $data_gallery_name_en = $row['gallery_name_en'];
						  
						  $mainimagename = $row['gallery_image'];
						  if($mainimagename != '')
						  {
						  	  if(file_exists("../../../upload/".$mainimagename) )
							  {
			  	  ...
			  	  $smarty->assign('data_gallery_description_id' ,  $data_gallery_description_id); 
			  	  $smarty->assign('data_gallery_description_en' ,  $data_gallery_description_en); 
			  	  $smarty->assign('data_gallery_name_id' ,  $data_gallery_name_id); 
			  	  $smarty->assign('data_gallery_name_en' ,  $data_gallery_name_en); 
			  	  
			  	  //*** View Gallery
			  	  $sql = 'SELECT * FROM `d_companyvalues_subcontent` WHERE `'.$primarykey.'` =\''.$data_primarykey.'\'   ORDER BY gallery_id ASC ';
			  	  $res = mysql_query($sql);
			  	  $num = @mysql_num_rows($res);
			  	  if($num)
			  	  {  $i=0;
				  	  while($row = mysql_fetch_assoc($res))
				  	  {   $gallery_gallery_image[$i] = is_file('../../../upload/thumb_'.$row['gallery_image']) ? '<img src="../../../upload/thumb_'.$row['gallery_image'].'" alt="'.$row['gallery_image'].'"  width="150"  />' : '&nbsp;';
					  	  $gallery_gallery_description_id[$i] =  $row['gallery_description_id'] ;
					  	  $gallery_gallery_id[$i] = $row['gallery_id'];
					  	  $gallery_gallery_name_id[$i] = $row['gallery_name_id'];
					  	  $gallery_gallery_ismain[$i] = $row['gallery_ismain'] == 1 ? '<span class="label bg-primary">Main Image</span>' : 'N/A'; 
					  	  $gallery_delete_link[$i] = basename(__FILE__).'?action=subcontent&data_primarykey='.$data_primarykey.'&delete_img='.$row['gallery_id'].'&old_mainimage='.$row['gallery_image'].'';
					  	  $gallery_edit_link[$i] = basename(__FILE__).'?action=subcontent&data_primarykey='.$data_primarykey.'&edit_img='.$row['gallery_id'].'';
					  	  $i++;
				  	  }
				  	  $smarty->assign('gallery', array(
				  	   'gallery_gallery_image' =>  $gallery_gallery_image,
				  	   'gallery_gallery_description_id' =>  $gallery_gallery_description_id,
				  	   'gallery_gallery_id' =>  $gallery_gallery_id,
				  	   'gallery_gallery_ismain' =>  $gallery_gallery_ismain,
				  	   'gallery_delete_link' =>  $gallery_delete_link,
				  	   'gallery_edit_link' =>  $gallery_edit_link,
	  ...
	  $smarty->assign('msg' , $msg); 
	  $smarty->display($tplfile.'_subcontent.tpl');
	break;
