...
$imagethumbsize_x = 423;
$imagethumbsize_y = 461;
$best_image_view = '<br />Best view: '.($imagethumbsize_x  ).'px - '.($imagethumbsize_y  ).'px';
$smarty->assign('best_image_view' ,  $best_image_view);

$imagethumbsize_x2 = 296;
$imagethumbsize_y2 = 220;
$best_image_view2 = '<br />Best view: '.($imagethumbsize_x2  ).'px - '.($imagethumbsize_y2  ).'px';
$smarty->assign('best_image_view2' ,  $best_image_view2);
...
{	
	...
	{ 
	  ...
	  $data_content_email 		= trim($_POST["data_content_email"]);  
	  
	   
	  //** Validasi inputan user **// 
	 $data_d_newsmain_date_sql =   date('Y-m-d H:i:s');
		   ...
		   if (isset($_POST["insert"])) 
	  	   {
			   ...
			   content_email='".mysql_real_escape_string($data_content_email)."',   
		...
		if ($page != 0 ) {
	      ...
	      while (!$RS->EOF) {
		      ...
		      $link_p = isset($_GET['page']) ? 'page='.$_GET['page'] : '';
	  	      ...
	  	      $d_slideshow_statname_temp[$i] = $arr_showhide_name[$RS->fields['content_status']]; 
	  	      $d_slideshow_status_temp[$i] = $RS->fields['content_status']; 
	  	      $i++;
	  	      $RS->MoveNext();
	  	  }
