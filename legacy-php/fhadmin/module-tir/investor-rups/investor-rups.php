			  ...
			  if(is_array($RS->fields) && sizeof($RS->fields)>0)
			  {
				  ...
				  { 
					 ...
					 if($data_gallery_name_id == '' || $data_gallery_name_en == '')
					  ...
					  if($errmsg == '')
					  {
						  if(isset($_POST['submitedit']))
						  {
							  $sql = 'UPDATE  `d_rupspdf` SET `'.$primarykey.'`=\''.$data_primarykey.'\', `gallery_pdf`=\''.$mainimagename.'\', `gallery_pdf2`=\''.$mainimagename2.'\',  `gallery_name_id`=\''.$data_gallery_name_id.'\',   `gallery_name_en`=\''.$data_gallery_name_en.'\'
						  ...
						  {
							  $sql = 'INSERT INTO  `d_rupspdf` SET `'.$primarykey.'`=\''.$data_primarykey.'\', `gallery_pdf`=\''.$mainimagename.'\', `gallery_pdf2`=\''.$mainimagename2.'\',  `gallery_name_id`=\''.$data_gallery_name_id.'\',   `gallery_name_en`=\''.$data_gallery_name_en.'\'  ;';
				  ...
				  if(isset($_GET['delete_img']))
			  	  {  $old_mainimage = $_GET['old_mainimage'];
				  	  ...
				  	  $sql = 'DELETE FROM `d_rupspdf`  WHERE gallery_id=\''.intval($_GET['delete_img']).'\' ;';
			  	  ...
			  	  {   
				  	  $sql = 'SELECT * FROM `d_rupspdf`  WHERE gallery_id=\''.intval($_GET['edit_img']).'\' ;';
					  $res = mysql_query($sql);
				  	  $num = @mysql_num_rows($res);
				  	  if($num)
				  	  {
			  	  ...
			  	  //*** View Gallery
			  	  $sql = 'SELECT * FROM `d_rupspdf` WHERE `'.$primarykey.'` =\''.$data_primarykey.'\'   ORDER BY gallery_id ASC ';
			  	  $res = mysql_query($sql);
			  	  $num = @mysql_num_rows($res);
			  	  if($num)
			  	  {  $i=0;
