...
$imagethumbsize_x = 285;
$imagethumbsize_y = 380;
...
{	
	...
	$option_stock_val = array(1, 0); 
	$arr_stock_name = array( 1=> 'Tersedia', 0 => 'Stock kosong');
	$smarty->assign('option_stock_val' ,$option_stock_val); 
	$smarty->assign('arr_stock_name' ,$arr_stock_name); 
	...
	{ 
	  ...
	  $data_d_content_video 			= trim($_POST["data_d_content_video"]); 
	  ...
	  $data_mainpdf 				= $_FILES["data_mainpdf"];
	  $old_mainpdfname 			= trim($_POST["old_mainpdfname"]);
	  if ($data_mainpdf['name'] == "" && $old_mainpdfname == "" && $requiremainpdf === true) { $msg.="Please input PDF<br>";}

	  if($allowmainpdf === true && isset($data_mainpdf["name"]) && $data_mainpdf['name'] !='')
	  {
		   	$handle                     = new Upload($data_mainpdf);
	        $handle->allowed            = array('application/pdf'); 
	        if ($handle->uploaded) { 
	            $handle->Process("../../../upload/");
	            if ($handle->processed) {
	                
	                $mainpdfname     = $handle->file_dst_name;
	  ...
	  {
		  $mainpdfname = isset($_POST["insert"]) ? $old_mainpdfname : '';
	  }
		   ...
		   if (isset($_POST["insert"])) 
	  	   {
			   ...
			   if($data_priority != $_POST['data_old_priority'] )
				...
				if ($allowmainpdf === true && isset($mainpdfname) && $mainpdfname != '') {
	                if (file_exists('../../../upload/'. $old_mainpdfname)) {
	                    @unlink('../../../upload/'. $old_mainpdfname);
	                }
	                
	                $db->Execute("UPDATE " . $maintable . " SET mainpdfname='" . $mainpdfname . "' WHERE " . $primarykey . " = " . $data_primarykey);
	            }
	...
	case "insert" :
	  ...
	  if($allowmainpdf === true)
	  {
		  if($mainpdfname != '')
		  {
		  	  if(file_exists("../../../upload/".$mainpdfname) )
			  {
				  $oldpdf = '<div class="row"> 
	                          <div class="form-group">
	                        <label class="col-sm-2 control-label">&nbsp;</label>
	                        <div class="col-sm-6"><BR />
	                        <a href="../../../upload/'.$mainpdfname.'" target="_blank">'.$mainpdfname.'</a> <input type="hidden" name="old_mainpdfname" value="'.$mainpdfname.'" />
	                        </div> 
	                        </div></div>';
				   
			  }  
			  $smarty->assign('oldpdf' 	, $oldpdf);
	  	  } 
  	  } 
	...
	case "detail" :
	  ...
	  if(is_array($RS->fields) && sizeof($RS->fields)>0)
	  {
	  	  ...
	  	  if($allowmainpdf === true)
		  {
			  ...
			  if(isset($RS->fields["mainpdfname"]) && $RS->fields["mainpdfname"] != '' && file_exists("../../../upload/".$RS->fields["mainpdfname"]) )
			  {
				 $oldpdf = '<div class="row"> 
	                          <div class="form-group">
	                        <label class="col-sm-2 control-label">&nbsp;</label>
	                        <div class="col-sm-6"><BR />
	                        <a href="../../../upload/'.$RS->fields["mainpdfname"].'" target="_blank"> '.$RS->fields["mainpdfname"].' </a>  <input type="hidden" name="old_mainpdfname" value="'.$RS->fields["mainpdfname"].'" />
	                        <br />
	                        <a href="pdf.php?remove&i='.$RS->fields[$primarykey].'&p='.$primarykey.'&t='.$maintable.'&f='.basename(__FILE__).'" style="color: blue;"><span class="label bg-danger">Remove pdf</span></a>
	                        </div> 
	                        </div></div>'; 
			  } 
			  $smarty->assign('oldpdf' 	, $oldpdf);
