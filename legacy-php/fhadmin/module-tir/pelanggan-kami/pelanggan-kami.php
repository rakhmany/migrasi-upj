...
{	
	...
	case "search" :
	  if ($search!="") {
		  $sql_search = ' WHERE d_slideshow_name_id like \'%'.$search.'%\' ';
	  }
	    $SQL = "SELECT 	*
	  	   	  FROM		".$maintable." ".$sql_search."
	  ...
	  { $total_jumlah_record = $RS->RecordCount();
   	  	...
   	  	$teks_parameter = "&action=view&order=".$order."&orderfield=".$orderfield."&search=".$search;
