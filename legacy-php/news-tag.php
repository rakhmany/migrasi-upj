...
$html_page_title  				=   $language == '_en' ? 'News' : 'Berita' ; 
$fh_general_metakeyword 		= $html_page_title;
$fh_general_metadescription 	= $html_page_title;


$kataterkaitid = intval($_GET['id']);
$sql = ' SELECT * FROM `latest_news1_kataterkait` WHERE    kataterkaitid=\''.$kataterkaitid.'\'  limit 1  ';
$RS = $db->Execute($sql); 
        ...
        <section id="page-content">
            <div class="container">
                <h2><?php echo  $data_details['kataterkait']; ?></h2>
                   ...
                   <?php
                    $sql = ' SELECT * FROM `latest_news1` LEFT JOIN latest_news1_kataterkait ON latest_news1_kataterkait.newsid=latest_news1.newsid WHERE    latest_news1.newsstatus  != \'Hidden\' AND kataterkait='.$db->qstr($data_details['kataterkait']).' ORDER BY  latest_news1.`newsdate` DESC,   latest_news1.`newsid` DESC ';
					$RS = $db->Execute($sql); 
					if ($RS->fields['newsid'] != "")
					{
						$num 			= $RS->RecordCount();
						...
						$uparam 		= "";
						$perpage 		= $num;
						$pagination 	= pagingfront($perpage, $num, $page, $uparam, 'news/page/');
						//$pagination 	= pagingfront($perpage, $num, $page, $uparam, 'news/tag/'.$RS->fields['kataterkaitid'].'/'.createPermaLink($RS->fields['kataterkait']).'/page/');
						echo '<div id="blog" class="grid-layout post-3-columns m-b-30" data-item="post-item">';
						$sql = ' SELECT * FROM `latest_news1` LEFT JOIN latest_news1_kataterkait ON latest_news1_kataterkait.newsid=latest_news1.newsid WHERE    latest_news1.newsstatus  != \'Hidden\' AND kataterkait='.$db->qstr($data_details['kataterkait']).' ORDER BY  latest_news1.`newsdate` DESC,   latest_news1.`newsid` DESC  limit '.$pagination['from'].',  '.$perpage.'  ';
						$RS = $db->Execute($sql); 
						if ($RS->fields['newsid'] != "")
						{		
							while (!$RS->EOF)  
