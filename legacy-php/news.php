        ...
        <section id="page-content">
            <div class="container">
                ...
                <div id="blog" class="grid-layout post-3-columns m-b-30" data-item="post-item">
                   <?php
                    $sql = ' SELECT * FROM `latest_news1` WHERE    newsstatus  != \'Hidden\' ORDER BY  `newsdate` DESC,   `newsid` DESC ';
					$RS = $db->Execute($sql); 
					if ($RS->fields['newsid'] != "")
					{
						$num 			= $RS->RecordCount();
						...
						$sql = '  SELECT * FROM `latest_news1` WHERE    newsstatus  != \'Hidden\' ORDER BY  `newsdate` DESC,   `newsid` DESC  limit '.$pagination['from'].',  '.$perpage.'  ';
						$RS = $db->Execute($sql); 
						if ($RS->fields['newsid'] != "")
						{		
							while (!$RS->EOF)  
